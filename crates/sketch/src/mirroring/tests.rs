...
fn a_selection_mirrored_across_an_axis_lands_the_other_side_of_it() {
    ...
    let made = sketch
        .mirror(&held, MirrorAxis::Sketch(SketchAxis::V))
...
fn a_trait_of_the_drawing_serves_as_the_axis() {
    ...
    let made = sketch
        .mirror(&[side], MirrorAxis::Trait(axis))
...
fn the_trait_serving_as_the_axis_is_not_copied_onto_itself() {
    ...
    let made = sketch
        .mirror(&[Element::Segment(axis), side], MirrorAxis::Trait(axis))
        .expect("a trait to mirror across");
...
fn a_mirrored_curve_still_turns_the_way_it_looks() {
    ...
    let made = sketch
        .mirror(&[Element::Arc(arc)], MirrorAxis::Sketch(SketchAxis::V))
        .expect("an axis to mirror across");
...
fn a_circle_keeps_its_size_across_the_axis() {
    ...
    let made = sketch
        .mirror(&[Element::Circle(round)], MirrorAxis::Sketch(SketchAxis::V))
        .expect("an axis to mirror across");
...
fn a_trait_with_no_length_cannot_be_an_axis() {
    ...
    assert_eq!(sketch.mirror(&[side], MirrorAxis::Trait(nowhere)), None);
}
