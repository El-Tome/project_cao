...
Files written in an earlier format are not read: the tool has changed too much
for a conversion to be trustworthy, and nothing precious was drawn with those
versions. See [history.md](history.md).
...
- **A per-step geometry cache.** Not implemented. The whole part has one —
  ...
  — and whenever it does not answer, the whole design is replayed from
  nothing. Each major step now has a folder of its own in `design/`, which is
  where the geometry it rebuilds to would go: invalidated when that step's own
  operations change, and always rebuildable from them.
